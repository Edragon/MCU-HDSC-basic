...
uint8_t QMC5883_disable(void)
{
	uint8_t Temp;
	
	QMC5883_WriteReg(0x0c, 0x09);/****OSR=512,RNG=+/-2G,ODR=200Hz,MODE= continuous*******/
    return 0;
}

uint8_t QMC5883_enable(void)
{
	uint8_t Temp;
...
/******************************************************************************
* @函数名称：QMC5883_algo_angle(float *Magnet)
* @函数描述：计算角度
* @输入参数：地磁Mag xyz raw data
* @输出参数：None
*******************************************************************************/
QMC5883_algo_angle(float *Magnet)
{
    float angle_yaw, angle_pitch,angle_roll;
    
    angle_yaw = atan2((double)MagnetRawAd[1],(double)MagnetRawAd[0]) * (180 / 3.14159265) + 180;
